...
const AllSubscriptions = () => {
  ...
  useEffect(() => {
    const fetchSubscriptions = async () => {
      try {
        const {data} = await axios.get(`/api/v1/subscriptions/get-subscribed-channels/${user._id}`);
      ...
      <div className="flex">
        ...
        <div className="p-6 px-10 ml-15 w-full">
          ...
          {subscriptions.map((subscription) => (
            <div key={subscription._id} className="w-full flex items-center justify-between mb-4">
              <div className="rounded-full min-w-40 h-40 content-center cursor-pointer">
                <img src={subscription?.avatar?.url} alt="avatar" className="w-full h-full rounded-full"/>
              </div>

              <div className="w-full pl-4">
                <h2 className="text-xl mb-3 cursor-pointer">{subscription.fullname}</h2>
